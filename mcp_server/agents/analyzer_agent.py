...
def analyze_task(task_text: str, repo_dir: str | Path = ".") -> Dict[str, object]:
    """Lightweight analyzer that suggests relevant files by keyword search.

    - Uses ripgrep if available; otherwise falls back to heuristics.
    """
    ...
    return {"task": task_text, "suggested_files": unique[:50], "plan": plan}
