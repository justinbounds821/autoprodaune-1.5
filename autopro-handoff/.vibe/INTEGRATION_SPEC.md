...
|-----------------------------|-----------------------------------|----------------------------------------|--------|-------------------------------------------------|-----------|------------------------------------------|
| `/admin/automation`         | Vizualizare status automation     | `GET /api/automation/status`           | GET    | – → `{ enabled: bool, last_run: str, ... }`     | **ok**    | CORS fixed: :3006/:3007 added            |
|                             | Toggle automation on/off          | `POST /api/working-automation/toggle`  | POST   | `{active: bool}` → `{ok: true, data: {...}}`    | **ok**    | Using working_automation.py route        |
...
|                             | Actualizare plată                 | `PUT /api/financial/payments/{id}`     | PUT    | `{amount, status, ...}` → `{ok: true}`          | **ok**    | Connected via PaymentTracker.tsx         |
|                             | Ștergere plată                    | `DELETE /api/financial/payments/{id}`  | DELETE | – → `{ok: true}`                                 | **ok**    | Connected via PaymentTracker.tsx         |
|                             | Overview (stats)                  | `GET /api/financial/payments/overview` | GET    | `?period=30d` → `{total_paid, total_pending, ...}` | **ok** | |
| `/admin/video` (HeyGen)     | Lista avatare disponibile         | `GET /api/video/video/heygen/avatars`  | GET    | – → `{items: [...]}` sau `400 {detail: "HEYGEN_API_KEY..."}` | **ok**    | UX banner implemented when key missing   |
|                             | Generare video nou                | `POST /api/video/video/heygen/generate`| POST   | `{script: str, quality?: str, style?: str}` → `{id: str, status: "pending"}` | **ok**    | UX banner + disabled button when no key  |
