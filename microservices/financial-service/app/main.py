...
setup_logging("financial-service", level=os.getenv("LOG_LEVEL", "INFO"))
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Financial Service starting up...")
    ...
    try:
        ...
        logger.info("✅ Financial Service ready")
    ...
    logger.info("🛑 Financial Service shutting down...")
    try:
        db = get_database()
        await db.close()
        cache = get_redis()
...
metrics = setup_metrics(app, "financial-service")
health = init_health_check("financial-service")
