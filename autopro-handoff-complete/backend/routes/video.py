...
async def generate_heygen_video(
    ...
    try:
        ...
        result = await service.generate_video(heygen_request)
        
        if not result.success:
            raise HTTPException(
                status_code=500,
                detail=f"HeyGen API error: {result.error_message}"
            )
        
        # Estimează cost
        estimated_cost = service.estimate_cost(heygen_request)
        
        logger.info(f"[HeyGen] Video generation started: {result.video_id}")
        ...
        return {
            ...
            "style": style,
            "quality": quality,
            "check_status_url": f"/api/video/heygen/status/{result.video_id}"
        }
